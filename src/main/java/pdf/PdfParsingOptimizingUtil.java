 ...
 */
@Slf4j
public class PdfParsingOptimizingUtil {

    // 阈值0-255
    private static int YZ = 150;
     ...
     *
     */
    public static List<String> convertImage(PDDocument pdDocument, String filePath) {
        ...
        iterable.forEach((t)-> {
                ...
                try {
                    ...
                    if (pdResources.isImageXObject(t)) {
                            ...
                            String imagePath = filePath+System.getProperty("file.separator")+realName;
                            File file = new File(imagePath);
                        if (!file.getParentFile().exists()) {
                            file.getParentFile().mkdirs();
                        }
                            ImageIO.write(image, suffix, new File(imagePath));
//                            if(!ImageBinaryUtil.isBinaryCode(ImageBinaryUtil.file2MultipartfFile(file))){
//                                file.delete();
//                            }
                            list.add(imagePath);
                            }
